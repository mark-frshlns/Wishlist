...
const ItemQuantity = (props)=>{
      ...
      return (
        ...
        <div className="input-group input-group-small quantity">
          <button  type="submit" className="btn text-center" id={`subt`} onClick={()=>{props.onClick(props.item.id, "subt", max)}}><span className="fa fa-minus "/></button>
          ...
          <button  type="submit" className="btn  text-center"  id={`add`} onClick={()=>{props.onClick(props.item.id, "add", max)}}><span className="fa fa-plus " /></button>        
        </div>
